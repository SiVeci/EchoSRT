...
async def get_models():
    ...
    standard_models, en_models, distil_models = [], [], []
    for m in models:
        if "distil" in m: distil_models.append(m)
        elif m.endswith(".en"): en_models.append(m)
        else: standard_models.append(m)
...
async def get_llm_models(api_key: str, base_url: str = "https://api.openai.com/v1"):
    ...
    try:
        ...
        raise HTTPException(status_code=e.code, detail=f"获取失败: {e.read().decode('utf-8')}")
    except Exception as e:
...
async def get_asr_models(api_key: str, base_url: str = "https://api.openai.com/v1"):
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as response:
            data = json.loads(response.read().decode('utf-8'))
            all_model_ids = [m["id"] for m in data.get("data", []) if "id" in m]
            asr_keywords = ["whisper", "asr", "audio", "speech", "stt", "sensevoice", "paraformer"]
            filtered = [m_id for m_id in all_model_ids if any(kw in m_id.lower() for kw in asr_keywords)]
            return filtered if filtered else all_model_ids
    except urllib.error.HTTPError as e:
        raise HTTPException(status_code=e.code, detail=f"获取失败: {e.read().decode('utf-8')}")
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"拉取模型列表异常: {str(e)}")