...
- **双引擎 ASR 识别**：
  ...
  - *云端 API 引擎*：兼容标准的 OpenAI Audio API 及第三方代理，支持超大音频 (25MB+) 自动物理切片推流，并支持说话人识别与词级时间戳等高阶特性。
    - *⚠️ 算法局限提醒：处理大于 25MB 的音频时，云端模式会采用基于固定时间的“物理一刀切”策略（如每 10 分钟切片）。若连续发音恰好横跨切片毫秒点，小概率会导致拼接处吞字或产生局部幻觉。如需追求绝对连贯的广播级精度，建议优先使用支持流式长音频的本地引擎。*
